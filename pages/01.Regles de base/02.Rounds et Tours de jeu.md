...
On recrée le **Deck d'exploration**. On réintègre pour cela les cartes piochées lors du Round précédent, et qui ne sont pas conservées en jeu (capacités spéciales, effets du scénario, ...). Le scénario peut également laisser, sous condition, certaines cartes de côté. Tant que la condition n'est pas remplie, on ne les ajoute pas au **Deck d'exploration**.

## Initiative

Une fois le _Deck d'exploration_ constitué, chaque joueur va tirer sa carte d'initiative. Si une capacité permet de piocher plusieurs cartes, ces cartes sont piochées en même temps. Puis, pour chaque _groupe actif_ (ennemis, alliés, environnement, ...) présent en jeu, on pioche également une carte d'initiative que l'on attribue au groupe. Dans le cas où un groupe possède une capacité nécessitant la pioche de plusieurs cartes d'initiative, on pioche toutes les cartes en même temps, puis on suit les règles de la capacité pour gérer ces cartes multiples. On passe ensuite au groupe suivant.

Une fois que les cartes d'initiative des groupes actifs ont été attribuées et révélées, les joueurs disposant de plusieurs cartes d'initiative peuvent choisir celle qu'ils utiliseront et défaussent les cartes en trop (ou les gardent, selon leur capacité).
...
Un personnage ou un groupe actif peut disposer de plusieurs cartes d'initiative pour un round de jeu. Cela signifie qu'il dispose d'une initiative multiple. Il pourra joués plusieurs tours de jeu lors du round, aux différentes initiatives tirées.
...
Le _Joker_ peut être conservé pour bénir ou maudire un Jet de Compétence. Dans ce cas, si le joueur n'a pas d'autres cartes d'initiative, il doit en tirer une nouvelle pour déterminer son _Initiative_ du Round. Sinon il utilise une autre de ses cartes d'initiative (et peut dans ce cas perdre un tour de jeu en cas d'initiative multiple). S'il tire un nouveau _Joker_, il peut décider de l'utilisation qu'il en fait. Le joueur conserve la carte Joker jusqu'à son utilisation. L'utilisation d'un joker pour bénir ou maudire un Jet de Compétence doit être faite avant le lancer de dés. La carte est ensuite défaussée, et sera réintégrée au deck d'exploration à la prochaine phase de _Préparation_.
...
En suivant l'ordre décroissant des cartes (As, Roi, Dame, Valet, 10, 9, 8, 7, 6, 5, 4, 3, 2), puis l'ordre des couleurs (Coeur, Carreau, Pique, Trèfle), chaque personnage et groupe actif va jouer à tour de rôle.
...
Un _groupe actif_ est un ensemble de figurines ou d'éléments de décor qui agissent lors d'un Round de jeu tous ensembles. Il peut s'agir d'un groupe d'orques qu'affrontent les héros, d'une jeune fille qu'ils viennent sauver d'un sacrifice, ou de statues gardiennes d'un temple caché.

Lorsque vient le _tour de jeu_ d'un groupe, le dernier joueur ayant joué son personnage prend généralement les commandes du groupe. Certains groupes (généralement alliés) indiquent dans leur descriptif qui s'en occupe (généralement le joueur du personnage qui les dirige). Le joueur qui dirige le groupe doit suivre les directives de l'IA du groupe, mais il aura également parfois des choix à faire, ainsi que des lancers de dés.
