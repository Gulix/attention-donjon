...
En suivant l'ordre décroissant des cartes (As, Roi, Dame, Valet, 10, 9, 8, 7, 6, 5, 4, 3, 2), puis l'ordre des enseignes (Coeur, Pique, Carreau, Trèfle), chaque personnage et groupe actif va jouer à tour de rôle.
...
Plutôt que d'attendre, la meilleure solution est généralement d'agir.

Lorsqu'un joueur agit, il prend le nombre de dés auquel il a le droit. Ce nombre dépend de sa _Santé_, de ses _États_, de son _Équipement_. **L'Arcane Gardienne** ainsi que la carte d'initiative du personnage peuvent également influencer le nombre de dés à lancer.

Une fois les dés récupérés, le joueur choisit sa _Colonne d'action_. Un personnage débutant n'aura qu'une seule colonne d'action. Cette phase peut alors être sautée. S'il dispose de plusieurs _Colonnes d'action_, il doit en choisir une. Le personnage pourra utiliser toutes les actions de la colonne sélectionnée, ainsi que celles des colonnes à droite. Certaines colonnes ont des restrictions quant à leur usage, voir la section spécifique pour cela.

Le joueur jette l'ensemble des dés, puis peut appliquer le pouvoir de la colonne sélectionnée pour éventuellement relancer vorie rajouter des dés. Une fois la relance effectuée, il dispose d'une réserve de dés qu'il doit affecter aux actions à sa disposition. Tous les dés doivent être affectés. On résout ensuite les actions.
